...
def extract_panorama_from_e57():
    # Find E57 files in main data/input folder
    script_dir = os.path.dirname(__file__)  # ...panorama
    processing_dir = os.path.dirname(script_dir)  # ...point_cloud_processing
    repo_root = os.path.dirname(processing_dir)  # ...iris
    
    input_dir = os.path.join(repo_root, "data", "input")
    output_dir = os.path.join(processing_dir, "data", "panoramas")
    ...
    for e57_path in e57_files:
        ...
        panorama_output = os.path.join(output_dir, f"{e57_basename}_panorama.jpg")
        
        # Check if panorama already exists
        if os.path.exists(panorama_output):
            print(f"  ✓ Panorama already exists, skipping")
            continue
